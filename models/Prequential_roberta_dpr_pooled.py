...
def load_dataframes(data_dir):
    ...
    df = []
    df.append(pd.read_pickle(os.path.join(data_dir, f"train_w_dpr_ranked_rel_processed.pkl")))
    df.append(pd.read_pickle(os.path.join(data_dir, f"valid_w_dpr_ranked_rel_processed.pkl")))
    df.append(pd.read_pickle(os.path.join(data_dir, f"test_w_dpr_ranked_rel_processed.pkl")))
...
class ClaimData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        evidence_sents = str(self.evidence_sents[index])
...
def build_batch(df, batch_indices, max_seq_len, tokenizer):
    ...
    for index in batch_indices:
        ...
        evidence.append(" ".join(df.evidence_sents[index]))
